#!/usr/bin/env python3
"""test_pty.py — Phase 32 PTY smoke test.

Boots Aegis with q35 + NVMe disk (ext2 with pty_test binary),
logs in, runs /bin/pty_test, checks for "PTY OK" in output.
...
def run_test():
    ...
    try:
        ...
        # Run pty test
        time.sleep(1)
        _type_string(mon, "/bin/pty_test\n")

        if serial.wait_for("PTY OK", time.time() + CMD_TIMEOUT):
            print("PASS: pty_test reported PTY OK")
        else:
            print("FAIL: 'PTY OK' not found in output")
