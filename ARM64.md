...
**Status:** Kernel boots on `qemu-system-aarch64 -machine virt` through to the idle scheduler. No ARM64 userland yet — all stale blobs have been purged (see §14-§15). Next: build real aarch64-musl userland (phase A4).
...
## 15. First boot + stale userland purge (2026-04-12)

### First boot

After fixing the 12 initial blockers (§14) and the two link-stage issues (blockers A/B in §14), the ARM64 kernel booted on `qemu-system-aarch64 -machine virt -cpu cortex-a72 -m 2G -nographic -kernel build/aegis-arm64.elf`. Immediate panic at `sched_spawn` because `TPIDR_EL1` was never set — `percpu_current()` blindly dereferences whatever the firmware left behind. Root cause: `smp_percpu_init_bsp()` was declared in `kernel/arch/arm64/smp.h` but never implemented and never called (matches the CLAUDE.md "8 .o files missing" note). Fixed by:

- Defining `g_percpu[MAX_CPUS]` + `g_cpu_count` + `g_ap_online[]` in `kernel/arch/arm64/stubs.c`
- Implementing `smp_percpu_init_bsp()` — zero-initialize `g_percpu[0]`, `msr tpidr_el1, x0` + `isb`
- Calling it from `kernel_main()` right after `arch_init()`, before any scheduler call

Second boot reached `[SHELL] Aegis shell ready` and accepted interactive input. **This was a red herring.**

### The stale userland problem

`[SHELL] Aegis shell ready` is the startup banner of `/bin/sh`, which was replaced by `/bin/stsh` (Styx shell) in Phase 42 — 685 commits ago. The fact that this line appeared at all meant the kernel was running a 685-commit-old userspace snapshot.

The prior port embedded ARM64 user binaries as byte-array C files via `xxd -i`:

```
kernel/arch/arm64/init_arm64_bin.c     — PID 1 init (pre-vigil era)
kernel/arch/arm64/shell_arm64_bin.c    — /bin/sh
kernel/arch/arm64/login_arm64_bin.c    — login
kernel/arch/arm64/{ls,cat,echo,pwd,uname,clear,true,false,wc,grep,sort,
                   mkdir,touch,rm,cp,mv,whoami}_arm64_bin.c
```

20 files, 309,450 lines of byte arrays. They were generated by `tools/build-arm64-userland.sh`, a Docker-based build script that is itself stale — it hardcodes `/user/<name>/main.c` paths from before the `user/bin/` layout, references `/bin/shell` instead of `/bin/stsh`, and looks for an `init-arm64/init_musl.c` source that no longer exists.

This frozen userland masked all ARM64 regressions. It ran under our current kernel because the syscall translation table in `kernel/syscall/syscall.c` (the `#ifdef __aarch64__` block for remapping Linux ARM64 syscall numbers → Aegis x86 numbers) happened to be permissive enough. But the `#` prompt was lying: it wasn't actually exercising current master's userland at all.

### Purge

Removed:
- `kernel/arch/arm64/*_arm64_bin.c` — all 20 files (309,450 lines)
- `tools/build-arm64-userland.sh` — stale regenerator
- `kernel/arch/arm64/Makefile` ARCH_OBJS entries for the 20 `*_arm64_bin.o` objects + the pattern rule `$(BUILD)/%_arm64_bin.o: %_arm64_bin.c`
- `kernel/proc/proc.c` `#ifdef __aarch64__` block that spawned `init_elf` — replaced with a no-op that prints `[INIT] ARM64 userland not yet built — running idle only`
- `kernel/fs/initrd.c` `#ifdef __aarch64__` extern block + fixup loop — restored the x86-only `_binary_*_bin_*` externs and made the 6 user-binary entries in `s_files[]` conditional on `#ifndef __aarch64__`
- `s_nfiles` static constant — now computed at runtime via sentinel walk so the count stays honest as `#ifdef` slices change

Added:
- `.gitignore` entries for `kernel/arch/arm64/*_arm64_bin.c` and `kernel/arch/arm64/*_bin.c` so generated blobs never re-enter the tree
- Explanatory comments in the relevant sites

### Current boot state (post-purge)

```
[SERIAL] OK: PL011 UART initialized
[PMM] OK: 2048MB usable across 1 regions
...
[VFS] OK: initialized
[INITRD] OK: 26 files registered
[INIT] ARM64 userland not yet built — running idle only
[SCHED] OK: scheduler started, 1 tasks
```

`build/aegis-arm64.elf` dropped from 4.8MB → 1.1MB. 26 files in the initrd instead of 32 (6 user-binary entries excluded). `[SCHED] OK: 1 tasks` because only the idle task exists without an init process to spawn.

**This is the true state of the ARM64 port.** Kernel is real and honest; userland is nothing. Next step is phase A4 (build real aarch64-musl userland via crosstool-NG or Alpine Docker, rebuild vigil + stsh + coreutils from current master sources, embed them via the same `.bin + objcopy` pattern as x86).

### Lessons for phase A4

- **Never commit generated blobs.** They freeze in time and lie about correctness. `.gitignore` now catches this.
- **Use the same embedding pipeline as x86.** The top-level Makefile has `BLOB_COPY_RULE` that runs `objcopy -O elf64-<target>` on `.bin` files — drive the arm64 version through the same mechanism. Symbols will be `_binary_<name>_bin_start/end` automatically and `kernel/fs/initrd.c` stays arch-agnostic for that list.
- **No `#ifdef __aarch64__` for ARM64 blob symbols.** If it compiles on both arches via the same objcopy convention, no dispatch is needed.
