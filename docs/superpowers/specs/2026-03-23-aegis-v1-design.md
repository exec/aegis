...
**QEMU test:** `-machine q35` (already using compatible machine type). PCIe devices visible in config space. Serial output updated.

**Boot oracle change:** add `[PCIE] OK: enumeration complete, N devices` and `[ACPI] OK: MCFG+MADT parsed`.
...
4. Allocate Admin SQ + Admin CQ (64 entries each, 64-byte entries) — physical addresses written to `ASQ`/`ACQ` registers
...
disk:
    ...
    # mount and populate via e2tools or loop mount
...
3. Stop: `USBCMD.RS=0`. Reset: `USBCMD.HCRST=1`. Wait for both to clear.
...
6. Allocate Event Ring Segment (64 TRBs) + Event Ring Segment Table (1 entry), write `ERSTBA`/`ERDP`
7. Set `USBCMD.RS=1`
...
**VESA/GOP backend:** GRUB passes a multiboot2 framebuffer tag containing `framebuffer_addr` (physical), `framebuffer_pitch`, `framebuffer_width`, `framebuffer_height`, `framebuffer_bpp`. We map `framebuffer_addr` into kernel VA via the VMM window allocator and fill in `fb_info_t`. This works in QEMU day one with no additional flags — GRUB sets up the framebuffer via BIOS/UEFI before handing off to the kernel.

**Text renderer:** extracts the 8×16 glyph table from the VGA ROM font data already in `vga.c`. `fb_blit_glyph` draws one character as 128 pixels (8 wide × 16 tall) in the requested fg/bg colours.

**printk update:** gains a framebuffer path alongside existing serial and VGA text paths:
```c
if (fb_get_info() != NULL)
    fb_console_putchar(c);   /* scrolling framebuffer text console */
serial_putchar(c);           /* always */
if (!s_fb_active)
    vga_putchar(c);          /* only if no framebuffer */
```
...
**QEMU test:** existing tests unaffected — serial output oracle doesn't care about framebuffer. Add `test_fb.py` that boots and verifies the framebuffer tag was parsed (print `[FB] OK: ...` to serial).
...
**Files:**
- Create: `kernel/fb/fb_amddc.c` + `fb_amddc.h`
...
**DCN version detection:** PCI device ID determines DCN version:
- DCN 2.1: Ryzen 5000G (Cezanne), device IDs `0x1638`, `0x164C`, etc.
- DCN 3.0: Ryzen 6000 (Rembrandt), device IDs `0x1681` etc.
- DCN 3.1: Ryzen 7000 (Phoenix/Raphael iGPU), device IDs `0x164E` etc.
...
4. Allocate scanout framebuffer: carve a region from VRAM aperture aligned to 256-byte boundary
5. Configure HUBP (Hub and Prefetch): set surface address (VRAM physical), pitch, pixel format (ARGB8888)
6. Configure DPP (Display Pipe and Plane): identity color matrix, no scaling
7. Configure OPP (Output Pixel Processing): bypass LUT
8. Configure OPTC (Output Pipe and Timing Controller): program CRTC timings from EDID
9. Read EDID via DDC I2C (I2C master built into AMD DC register block): get native resolution and timings
...
# mount via losetup, format ESP as FAT32, install GRUB EFI
# format partition 2 as ext2, populate /bin, /etc, /tmp, /home
```
...
**`make run-disk`:** boots `aegis.img` in QEMU via NVMe (replaces ISO boot for full-stack testing):
```
...
**`make run-disk` (full disk boot, post-Phase 25):**
```
qemu-system-x86_64 -machine q35 -cpu Broadwell
    -drive file=build/aegis.img,if=none,id=nvme0
    -device nvme,drive=nvme0,serial=aegis0
    -device qemu-xhci -device usb-kbd
    -m 256M -serial stdio
    -display gtk    ← framebuffer visible
```
